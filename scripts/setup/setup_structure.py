...
def get_existing_hero_files(repo):
    ...
    while queue:
        ...
        for content in current:
            if content.type == "dir":
                queue.append(repo.get_contents(content.path))
            elif content.type == "file" and content.path.endswith(".json"):
                # Припустимо, структура: heroes/<Class>/<hero_name>.json
                parts = content.path.split('/')
                if len(parts) == 3:
                    _, hero_class, hero_file = parts
                    hero_name = hero_file.replace('.json', '').replace('_', ' ').title()
                    hero_files[hero_name] = hero_class
    return hero_files
            ...
            if hero in existing_hero_files:
                ...
                if current_class != correct_class:
                    ...
                    try:
                        ...
                        repo.update_file(hero_file_path, f"Update {hero_file_path}", json.dumps(hero_data, ensure_ascii=False, indent=4), file_content.sha)
