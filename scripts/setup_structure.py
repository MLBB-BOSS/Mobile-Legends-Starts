...
# Define repository structure excluding .github
structure = {
    "config": {
        "__init__.py": "",
...
""",
        "features": {
            "items.md": """# Items Feature Documentation

## Overview

The Items feature provides users with information about various in-game items available in Mobile Legends: Starts (MLS).

## Commands

- `/items` - Lists all available items.

## Data Structure

Items are stored in `data/items.json` as a list of item names.

## Implementation

The `/items` command is handled by `handlers/items.py`, which loads the items from `data/items.json` and sends them to the user.
""",
            "spells.md": """# Spells Feature Documentation

## Overview

The Spells feature allows users to view and learn about different spells available in Mobile Legends: Starts (MLS).
...
Spells are stored in `data/spells.json` as a list of spell names.

## Implementation

The `/spells` command is handled by `handlers/spells.py`, which loads the spells from `data/spells.json` and sends them to the user.
""",
            "emblems.md": """# Emblems Feature Documentation

## Overview

The Emblems feature provides information about different emblems that players can use to enhance their heroes in Mobile Legends: Starts (MLS).

## Commands

- `/emblems` - Lists all available emblems.

## Data Structure

Emblems are hardcoded in `handlers/emblems.py` as a list of emblem names.

## Implementation

The `/emblems` command is handled by `handlers/emblems.py`, which sends the list of emblems to the user.
""",
            "recommendations.md": """# Recommendations Feature Documentation

## Overview

The Recommendations feature suggests optimal and counter builds for heroes in Mobile Legends: Starts (MLS).

## Commands

- `/recommendations <hero_name>` - Provides build recommendations for the specified hero.

## Data Structure

Recommendations are generated by `utils/recommendations_engine.py` and returned as dictionaries containing optimal and counter builds.

## Implementation

The `/recommendations` command is handled by `handlers/recommendations.py`, which takes the hero name as an argument, fetches the recommended builds, and sends them to the user.
""",
            "comparisons.md": """# Comparisons Feature Documentation

## Overview

The Comparisons feature allows users to compare different heroes, items, or spells in Mobile Legends: Starts (MLS).

## Commands

- `/comparisons` - Provides various comparisons between heroes, items, or spells.

## Data Structure

Comparisons are hardcoded in `handlers/comparisons.py` as a list of comparison topics.

## Implementation

The `/comparisons` command is handled by `handlers/comparisons.py`, which sends the list of comparisons to the user.
""",
            "screenshots.md": """# Screenshots Feature Documentation

## Overview

The Screenshots feature enables users to upload and manage their in-game screenshots in Mobile Legends: Starts (MLS).

## Commands

- `/myscreenshots` - Displays the user's uploaded screenshots.

## Data Structure

Screenshots are stored in the `screenshots` folder and their URLs are saved in the database linked to the user.

## Implementation

- **Uploading Screenshots**: Users can send photos to the bot, which are handled by `handlers/screenshots_handler.py`. The screenshots are uploaded to AWS S3 using `services/s3_service.py`, and the URLs are saved in the database.
  
- **Viewing Screenshots**: The `/myscreenshots` command is handled by `handlers/myscreenshots_handler.py`, which retrieves and displays the user's uploaded screenshots.
"""
        },
    },
    "handlers": {
...
The Telegram ML Bot is structured to provide a modular and scalable architecture, allowing easy maintenance and expansion of features.

## Key Components

- **Handlers**: Manage different user commands and interactions. Each handler corresponds to a specific feature or command.
  
- **Utils**: Contains utility functions and helpers that are used across various parts of the application, such as data loading, API integrations, and template management.
  
- **Services**: Integrates with external services like AWS S3 for storage, Google Auth for authentication, and OpenAI for AI-driven features.
  
- **Models**: Defines the database schema using SQLAlchemy ORM, including models for users, screenshots, and badges.
  
- **Data**: Stores static data files such as badges, heroes, characters, prompts, and builds. These files are used to provide information to users and support the bot's functionality.
  
- **Scripts**: Includes deployment and setup scripts. The `setup_structure.py` script automates the creation of the repository structure and populates it with necessary files.
  
- **Tests**: Contains unit and integration tests to ensure the reliability and correctness of the application's functionality.
  
- **Docs**: Holds documentation files detailing the architecture, features, and usage of the bot.

## Data Flow

1. **User Interaction**: Users interact with the bot through Telegram commands and messages.
   
2. **Command Handling**: Commands are routed to the appropriate handler based on the user's input.
   
3. **Data Processing**: Handlers utilize utilities and services to process data, fetch information, and perform actions like uploading screenshots.
   
4. **Database Operations**: Models interact with the database to store and retrieve user data, screenshots, and badges.
   
5. **External Integrations**: Services handle interactions with external APIs and services, ensuring data is processed and stored correctly.
...
# List of heroes is loaded from 'heroes_data.json'
...
def create_folder(repo, path):
    try:
        ...
        if e.status == 404:
            # GitHub doesn't support empty folders, so add a .gitkeep file
...
def create_heroes(repo, heroes):
    for hero_class, hero_list in heroes.items():
        ...
        for hero in hero_list:
            ...
            # Check to avoid creating the same hero in multiple classes
            try:
                repo.get_contents(hero_file_path)
                print(f"File '{hero_file_path}' already exists.")
                continue
            except GithubException as e:
                if e.status != 404:
                    print(f"Error checking file '{hero_file_path}': {e}")
                    continue
            
            # Create basic content for the hero
            hero_data = {
                "name": hero,
                "class": hero_class,
                "role": "",  # Fill in the appropriate role if needed
                "skills": [],
                "builds": {
                    "optimal": [],
                    "counter": []
...
def create_structure(current_path, structure, repo):
    ...
    # After creating all folders and files, create the heroes folder
