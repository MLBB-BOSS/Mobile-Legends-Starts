...
class LocalizationManager:
    ...
    def _load_messages(self) -> dict:
        try:
            base_path = Path(__file__).parent.parent / "config/messages/locales"
            file_path = base_path / f"{self.locale}.json"
...
logger.info(f"Значення 'messages.unhandled_message': {loc.gimport json
from pathlib import Path
import logging
...
logger = logging.getLogger(__name__)
logger.info(f"Значення 'buttons.profile': {loc.get_message('buttons.profile')}")
logger.info(f"Значення 'messages.unhandled_message': {loc.get_message('messages.unhandled_message', message='Тестове повідомлення')}")et_message
