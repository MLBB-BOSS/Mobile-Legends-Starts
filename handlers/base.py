...
# Обробник натискання інлайн-кнопок для переходу між сторінками інструкцій
@router.callback_query(F.data.startswith("intro_next_") | F.data == "intro_start")
async def handle_intro_navigation(callback: CallbackQuery, state: FSMContext, bot: Bot):
    data = callback.data
    ...
    try:
        if data == "intro_next_1":
            # Перехід до другої сторінки інструкцій
            ...
            # Перехід до третьої сторінки інструкцій
            await bot.edit_message_text(
                chat_id=callback.message.chat.id,
                message_id=interactive_message_id,
        ...
        logger.error(f"Помилка при обробці інлайн-кнопки: {e}")
...
@router.callback_query(F.data == "mls_button" | F.data == "menu_back")
async def handle_inline_buttons(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        if data == "mls_button":
            ...
            # Оновлюємо інтерактивне повідомлення з описом головного меню
            await bot.edit_message_text(
                chat_id=callback.message.chat.id,
                message_id=interactive_message_id,
                text=MAIN_MENU_DESCRIPTION,
