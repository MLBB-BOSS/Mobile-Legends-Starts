...
async def handle_heroes(callback_query: CallbackQuery):
    ...
    elif action in ["fighter", "tank", "mage", "marksman", "assassin", "support"]:
        hero_class = menu_button_to_class.get(MenuButton(action.upper()).value)
        if hero_class:
            await callback_query.message.edit_text(
            ...
            await callback_query.answer("Клас не знайдено.", show_alert=True)
    else:
...
async def handle_hero_selection(callback_query: CallbackQuery):
    hero_name = callback_query.data.split("_", 1)[1]
