...
async def handle_back(message: Message, state: FSMContext, bot: Bot, target_state: State, target_text: str, target_keyboard: types.ReplyKeyboardMarkup):
    ...
    logger.info(f"Updated bot_message_id: {new_bot_message_id}")
    ...
    if bot_message_id:
        try:
            ...
            logger.info(f"Deleted previous bot message with ID: {bot_message_id}")
    ...
    if interactive_message_id:
        try:
            current_interactive = await bot.get_message(chat_id=message.chat.id, message_id=interactive_message_id)
            if current_interactive.text != target_text or current_interactive.reply_markup != get_generic_inline_keyboard():
                ...
                logger.info(f"Edited interactive message with ID: {interactive_message_id}")
            else:
                logger.info("Повідомлення не потребує редагування.")
        except Exception as e:
            ...
            logger.info(f"Updated interactive_message_id: {interactive_message.message_id}")

    # Встановлюємо новий стан
    await state.set_state(target_state)
    logger.info(f"Set new state to: {target_state}")
...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    logger.info("Set state to INTRO_PAGE_1")
    ...
    await state.update_data(interactive_message_id=interactive_message.message_id)
    logger.info(f"Saved interactive_message_id: {interactive_message.message_id}")
...
async def handle_intro_next_1(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        logger.info("Edited interactive message to INTRO_PAGE_2")
    except Exception as e:
        logger.error(f"Не вдалося редагувати інтерактивне повідомлення: {e}")
    ...
    logger.info("Set state to INTRO_PAGE_2")
    await callback.answer()
...
async def handle_intro_next_2(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        logger.info("Edited interactive message to INTRO_PAGE_3")
    except Exception as e:
        logger.error(f"Не вдалося редагувати інтерактивне повідомлення: {e}")
        await bot.send_message(
            chat_id=callback.message.chat.id,
    ...
    logger.info("Set state to INTRO_PAGE_3")
    await callback.answer()
...
async def handle_intro_start(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    logger.info(f"Saved bot_message_id: {main_menu_message.message_id}")
    ...
    try:
        ...
        logger.info("Edited interactive message to MAIN_MENU_DESCRIPTION")
    except Exception as e:
        logger.error(f"Не вдалося редагувати інтерактивне повідомлення: {e}")
        # Якщо не вдалося редагувати, відправляємо нове інтерактивне повідомлення
        interactive_message = await bot.send_message(
    ...
    logger.info("Set state to MAIN_MENU")
    await callback.answer()
...
async def handle_main_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if not bot_message_id or not interactive_message_id:
        ...
        await state.set_state(MenuStates.MAIN_MENU)
        logger.info("Set state to MAIN_MENU due to missing message IDs")
        return
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new main menu message with ID: {new_bot_message_id}")
    ...
    # Оновлюємо bot_message_id в стані
    await state.update_data(bot_message_id=new_bot_message_id)
    logger.info(f"Updated bot_message_id to: {new_bot_message_id}")

    # Редагуємо інтерактивне повідомлення (Повідомлення 2)
    try:
        current_interactive = await bot.get_message(chat_id=message.chat.id, message_id=interactive_message_id)
        if current_interactive.text != new_interactive_text or current_interactive.reply_markup != new_interactive_keyboard:
            await bot.edit_message_text(
                chat_id=message.chat.id,
                message_id=interactive_message_id,
        ...
        await state.update_data(interactive_message_id=interactive_message.message_id)
        logger.info(f"Sent new interactive message with ID: {interactive_message.message_id}")

    # Оновлюємо стан користувача
    await state.set_state(new_state)
    logger.info(f"Set new state to: {new_state}")
...
async def handle_navigation_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.HEROES.value:
        ...
        # Викликаємо допоміжну функцію handle_back
        await handle_back(
            message=message,
            state=state,
            bot=bot,
            ...
            target_keyboard=main_menu_keyboard
        )
        logger.info("Returned to MAIN_MENU from NAVIGATION_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
    ...
    # Зберігаємо новий bot_message_id
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new navigation menu message with ID: {new_bot_message_id}")
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        logger.info(f"Set hero_class to: {hero_class}")
        ...
        logger.info("Set state to SEARCH_HERO for hero search")
        ...
        new_state = MenuStates.HEROES_MENU
        logger.info("Comparison feature under development")
    elif user_choice == MenuButton.BACK.value:
        # Викликаємо допоміжну функцію handle_back для повернення до Навігації
        await handle_back(
            message=message,
            state=state,
            bot=bot,
            target_state=MenuStates.NAVIGATION_MENU,
            target_text=NAVIGATION_INTERACTIVE_TEXT,
            target_keyboard=get_navigation_menu()
        )
        logger.info("Returned to NAVIGATION_MENU from HEROES_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_interactive_text = "Невідома команда"
        new_state = MenuStates.HEROES_MENU
        logger.warning(f"Unknown command in HEROES_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new heroes menu message with ID: {new_bot_message_id}")
    ...
    try:
        current_interactive = await bot.get_message(chat_id=message.chat.id, message_id=interactive_message_id)
        if current_interactive.text != new_interactive_text or current_interactive.reply_markup != get_generic_inline_keyboard():
            await bot.edit_message_text(
                chat_id=message.chat.id,
                message_id=interactive_message_id,
                text=new_interactive_text,
...
async def handle_guides_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.NEW_GUIDES.value:
        ...
        logger.info("Selected BEGINNER_GUIDES")
        ...
        logger.info("Returned to NAVIGATION_MENU from GUIDES_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_interactive_text = "Невідома команда"
        new_state = MenuStates.GUIDES_MENU
        logger.warning(f"Unknown command in GUIDES_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new guides menu message with ID: {new_bot_message_id}")
...
async def handle_counter_picks_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.COUNTER_SEARCH.value:
        ...
        logger.info("Returned to NAVIGATION_MENU from COUNTER_PICKS_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_main_keyboard = get_counter_picks_menu()
        new_state = MenuStates.COUNTER_PICKS_MENU
        logger.warning(f"Unknown command in COUNTER_PICKS_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new counter picks menu message with ID: {new_bot_message_id}")
...
async def handle_builds_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.CREATE_BUILD.value:
        ...
        logger.info("Returned to NAVIGATION_MENU from BUILDS_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_main_keyboard = get_builds_menu()
        new_state = MenuStates.BUILDS_MENU
        logger.warning(f"Unknown command in BUILDS_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new builds menu message with ID: {new_bot_message_id}")
...
async def handle_voting_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.CURRENT_VOTES.value:
        ...
        logger.info("Set state to SEARCH_TOPIC for topic suggestion")
    elif user_choice == MenuButton.BACK.value:
        # Викликаємо допоміжну функцію handle_back для повернення до Навігації
        await handle_back(
            message=message,
        ...
        logger.info("Returned to NAVIGATION_MENU from VOTING_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_main_keyboard = get_voting_menu()
        new_state = MenuStates.VOTING_MENU
        logger.warning(f"Unknown command in VOTING_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new voting menu message with ID: {new_bot_message_id}")
...
async def handle_profile_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.STATISTICS.value:
        ...
        logger.info("Selected STATISTICS")
        ...
        logger.info("Selected ACHIEVEMENTS")
        ...
        logger.info("Returned to MAIN_MENU from PROFILE_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_main_keyboard = get_profile_menu()
        new_state = MenuStates.PROFILE_MENU
        logger.warning(f"Unknown command in PROFILE_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new profile menu message with ID: {new_bot_message_id}")
...
async def handle_inline_buttons(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if interactive_message_id:
        ...
        if data == "mls_button":
            ...
            logger.info("Handled mls_button callback")
        elif data == "menu_back":
            ...
            logger.info("Returned to MAIN_MENU from inline menu_back")
        else:
            # Додайте обробку інших інлайн-кнопок за потребою
            await bot.answer_callback_query(callback.id, text=UNHANDLED_INLINE_BUTTON_TEXT)
            logger.warning(f"Unhandled inline button callback: {data}")
    else:
        logger.error("interactive_message_id не знайдено")
        await bot.answer_callback_query(callback.id, text=GENERIC_ERROR_MESSAGE_TEXT)
        logger.info("Sent GENERIC_ERROR_MESSAGE_TEXT due to missing interactive_message_id")
...
async def handle_search_hero(message: Message, state: FSMContext, bot: Bot):
    ...
    if hero_name:
        ...
        logger.info(f"Hero search response: {response_text}")
    else:
        response_text = "Будь ласка, введіть ім'я героя для пошуку."
        logger.warning("Empty hero name entered for search")
    ...
    logger.info("Set state back to HEROES_MENU after hero search")
...
async def handle_search_topic(message: Message, state: FSMContext, bot: Bot):
    ...
    if topic:
        ...
        logger.info(f"Topic suggestion response: {response_text}")
    else:
        response_text = "Будь ласка, введіть тему для пропозиції."
        logger.warning("Empty topic entered for suggestion")
    ...
    logger.info("Set state to FEEDBACK_MENU after topic suggestion")
...
async def handle_change_username(message: Message, state: FSMContext, bot: Bot):
    ...
    if new_username:
        ...
        logger.info(f"Username change response: {response_text}")
    else:
        response_text = "Будь ласка, введіть новий Username."
        logger.warning("Empty username entered for change")
    ...
    logger.info("Set state to SETTINGS_MENU after username change")
...
async def handle_receive_feedback(message: Message, state: FSMContext, bot: Bot):
    ...
    if feedback:
        ...
        logger.warning("Empty feedback entered")
    ...
    # Повертаємо користувача до меню Зворотний Зв'язок
    await state.set_state(MenuStates.FEEDBACK_MENU)
    logger.info("Set state back to FEEDBACK_MENU after receiving feedback")
...
async def handle_report_bug(message: Message, state: FSMContext, bot: Bot):
    ...
    if bug_report:
        ...
        logger.info("Bug report received successfully")
    else:
        response_text = "Будь ласка, опишіть помилку, яку ви знайшли."
        logger.warning("Empty bug report entered")
    ...
    # Повертаємо користувача до меню Зворотний Зв'язок
    await state.set_state(MenuStates.FEEDBACK_MENU)
    logger.info("Set state back to FEEDBACK_MENU after reporting bug")
...
async def handle_help_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.INSTRUCTIONS.value:
        ...
        logger.info("Selected INSTRUCTIONS")
    elif user_choice == MenuButton.FAQ.value:
        new_main_text = FAQ_TEXT
        new_interactive_text = "FAQ"
        logger.info("Selected FAQ")
    elif user_choice == MenuButton.HELP_SUPPORT.value:
        new_main_text = HELP_SUPPORT_TEXT
        new_interactive_text = "Підтримка"
        logger.info("Selected HELP_SUPPORT")
    elif user_choice == MenuButton.BACK_TO_PROFILE.value:
        # Викликаємо допоміжну функцію handle_back для повернення до Профілю
        await handle_back(
            message=message,
            state=state,
            bot=bot,
            target_state=MenuStates.PROFILE_MENU,
            target_text=PROFILE_INTERACTIVE_TEXT,
            target_keyboard=get_profile_menu()
        )
        logger.info("Returned to PROFILE_MENU from HELP_MENU")
        return
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
        ...
        new_main_keyboard = get_help_menu()
        new_state = MenuStates.HELP_MENU
        logger.warning(f"Unknown command in HELP_MENU: '{user_choice}'")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new help menu message with ID: {new_bot_message_id}")
...
async def unknown_command(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state == MenuStates.MAIN_MENU.state:
        ...
        new_state = MenuStates.MAIN_MENU
        logger.info("Handled unknown command in MAIN_MENU")
    elif current_state == MenuStates.NAVIGATION_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_navigation_menu()
        new_interactive_text = "Навігаційний екран"
        new_state = MenuStates.NAVIGATION_MENU
        logger.info("Handled unknown command in NAVIGATION_MENU")
    elif current_state == MenuStates.HEROES_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_heroes_menu()
        new_interactive_text = "Меню Персонажі"
        new_state = MenuStates.HEROES_MENU
        logger.info("Handled unknown command in HEROES_MENU")
        ...
        new_state = MenuStates.HERO_CLASS_MENU
        logger.info(f"Handled unknown command in HERO_CLASS_MENU for class: {hero_class}")
    elif current_state == MenuStates.GUIDES_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_guides_menu()
        new_interactive_text = "Меню Гайди"
        new_state = MenuStates.GUIDES_MENU
        logger.info("Handled unknown command in GUIDES_MENU")
    elif current_state == MenuStates.COUNTER_PICKS_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_counter_picks_menu()
        new_interactive_text = "Меню Контр-піки"
        new_state = MenuStates.COUNTER_PICKS_MENU
        logger.info("Handled unknown command in COUNTER_PICKS_MENU")
    elif current_state == MenuStates.BUILDS_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_builds_menu()
        new_interactive_text = "Меню Білди"
        new_state = MenuStates.BUILDS_MENU
        logger.info("Handled unknown command in BUILDS_MENU")
    elif current_state == MenuStates.VOTING_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_voting_menu()
        new_interactive_text = "Меню Голосування"
        new_state = MenuStates.VOTING_MENU
        logger.info("Handled unknown command in VOTING_MENU")
    elif current_state == MenuStates.PROFILE_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_profile_menu()
        new_interactive_text = "Меню Профіль"
        new_state = MenuStates.PROFILE_MENU
        logger.info("Handled unknown command in PROFILE_MENU")
    elif current_state == MenuStates.STATISTICS_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_statistics_menu()
        new_interactive_text = "Меню Статистика"
        new_state = MenuStates.STATISTICS_MENU
        logger.info("Handled unknown command in STATISTICS_MENU")
    elif current_state == MenuStates.ACHIEVEMENTS_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_achievements_menu()
        new_interactive_text = "Меню Досягнення"
        new_state = MenuStates.ACHIEVEMENTS_MENU
        logger.info("Handled unknown command in ACHIEVEMENTS_MENU")
    elif current_state == MenuStates.SETTINGS_MENU.state:
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_settings_menu()
        new_interactive_text = "Меню Налаштування"
        new_state = MenuStates.SETTINGS_MENU
        logger.info("Handled unknown command in SETTINGS_MENU")
        ...
        logger.info("Sent navigation hint during input state")
        return
    else:
        ...
        new_state = MenuStates.MAIN_MENU
        logger.info("Handled unknown command in unknown state, returned to MAIN_MENU")
    ...
    new_bot_message_id = main_message.message_id
    logger.info(f"Sent new message with ID: {new_bot_message_id}")
...
def setup_handlers(dp):
    ...
    logger.info("Included base router into dispatcher")
