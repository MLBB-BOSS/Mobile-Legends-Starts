...
async def safe_edit_message(bot: Bot, chat_id: int, message_id: int, new_text: str, reply_markup=None):
    try:
        ...
        if "message is not modified" in str(e):
            logger.info("Повідомлення не було змінено. Ігнорую помилку.")
        else:
            logger.error(f"Помилка при редагуванні повідомлення: {e}")
            raise e  # Якщо це інша помилка, підніміть її

# Централізована функція для оновлення меню
async def update_menu(
    ...
    try:
        ...
        logger.error(f"Не вдалося відправити нове повідомлення з клавіатурою: {e}")
...
async def handle_intro_next_1(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        pass  # Помилка вже оброблена у safe_edit_message
    except Exception as e:
        logger.error(f"Не вдалося редагувати інтерактивне повідомлення: {e}")
        await bot.send_message(
            chat_id=callback.message.chat.id,
...
async def handle_main_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    # Викликаємо централізовану функцію для оновлення меню
    await update_menu(
        message=message,
        state=state,
        bot=bot,
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        new_main_text = UNKNOWN_COMMAND_TEXT
        new_main_keyboard = get_hero_class_menu(state_data.get('hero_class', 'Танк'))
        new_interactive_text = "Невідома команда"
        new_state = MenuStates.HEROES_MENU
