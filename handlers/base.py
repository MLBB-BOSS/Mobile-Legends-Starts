...
async def cmd_start(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    ...
    user = await get_user_profile(db, telegram_id, username=message.from_user.username)
...
async def handle_main_menu_buttons(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in main menu")
    ...
    # Fetch user profile for placeholders if needed
    user = await get_user_profile(db, telegram_id, username=message.from_user.username)
    ...
    if user_choice == MenuButton.NAVIGATION.value:
        ...
        # Заповнення плейсхолдерів в профілі
        profile_text = PROFILE_INTERACTIVE_TEXT.format(
            username=user.username or "Не вказано",
            level=25,  # Приклад: замініть на реальний рівень користувача
            rating=1500,  # Приклад: замініть на реальний рейтинг користувача
            achievements_count=10  # Приклад: замініть на реальну кількість досягнень
        )
        new_main_text = profile_text
        new_main_keyboard = get_profile_menu()
        new_interactive_text = PROFILE_INTERACTIVE_TEXT  # Можливо, потрібно змінити текст
...
async def handle_feedback_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Feedback Menu")
    ...
    if user_choice == MenuButton.SEND_FEEDBACK.value:
        ...
        new_interactive_text = "Введіть ваш відгук:"
        ...
        new_interactive_text = "Опишіть помилку, яку ви виявили:"
        new_main_keyboard = types.ReplyKeyboardRemove()
...
async def handle_navigation_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Navigation Menu")
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Heroes Menu")
    ...
    if user_choice in hero_classes:
        ...
        heroes_list = "\n".join(heroes_by_class.get(hero_class, []))
...
async def handle_guides_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Guides Menu")
...
async def handle_counter_picks_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Counter Picks Menu")
...
async def handle_builds_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Builds Menu")
...
async def handle_voting_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Voting Menu")
...
async def handle_statistics_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Statistics Menu")
    ...
    if user_choice == MenuButton.ACTIVITY.value:
        new_main_text = ACTIVITY_TEXT.format(activity_data="Ваші активності:")
        new_interactive_text = "General Activity"
    elif user_choice == MenuButton.RANKING.value:
        new_main_text = RANKING_TEXT.format(ranking_data="Ваше місце в рейтингу:")
        new_interactive_text = "Ranking"
    elif user_choice == MenuButton.GAME_STATS.value:
        new_main_text = GAME_STATS_TEXT.format(game_stats="Ваші статистики гри:")
...
async def handle_achievements_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Achievements Menu")
...
async def handle_settings_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Settings Menu")
...
async def handle_hero_class_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected '{user_choice}' in Hero Class Menu for class {hero_class}")
...
async def handle_help_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected {user_choice} in Help Menu")
...
async def handle_search_hero(message: Message, state: FSMContext, bot: Bot):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} is searching for hero: {hero_name}")
...
async def handle_search_topic(message: Message, state: FSMContext, bot: Bot):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} is suggesting a topic: {topic}")
...
async def handle_change_username(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} is changing username to: {new_username}")
    ...
    if new_username:
        try:
            ...
            logger.info(f"User {telegram_id} changed username to: {new_username}")
        except Exception as e:
            logger.error(f"Failed to change username for user {telegram_id}: {e}")
            response_text = "Виникла помилка при зміні username. Спробуйте пізніше."
    else:
...
async def handle_receive_feedback(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    feedback = message.text.strip()
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} sent feedback: {feedback}")
    ...
    if feedback:
        try:
            ...
            logger.info(f"Feedback from user {telegram_id} stored successfully.")
        except Exception as e:
            logger.error(f"Failed to store feedback from user {telegram_id}: {e}")
            response_text = "Виникла помилка при збереженні відгуку. Спробуйте пізніше."
    else:
...
async def handle_report_bug(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    ...
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} reported a bug: {bug_report}")
    ...
    if bug_report:
        try:
            ...
            logger.info(f"Bug report from user {telegram_id} stored successfully.")
        except Exception as e:
            logger.error(f"Failed to store bug report from user {telegram_id}: {e}")
            response_text = "Виникла помилка при збереженні звіту про помилку. Спробуйте пізніше."
    else:
...
async def handle_tournaments_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected '{user_choice}' in Tournaments Menu")
...
async def handle_meta_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected '{user_choice}' in META Menu")
...
async def handle_m6_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    user_choice = message.text
    telegram_id = message.from_user.id
    logger.info(f"User {telegram_id} selected '{user_choice}' in M6 Menu")
...
async def unknown_command(message: Message, state: FSMContext, bot: Bot):
    telegram_id = message.from_user.id
    logger.warning(f"Unknown message from {telegram_id}: {message.text}")
