...
from keyboards.menus import (
    ...
    get_heroes_menu, get_hero_class_reply_menu, get_guides_menu,
...
from keyboards.inline_menus import (
    get_generic_inline_keyboard,  # Можливо, ця функція тепер буде змінена або видалена
)
    ...
    if last_text != new_text or last_keyboard != new_keyboard:
        try:
            ...
            await bot.send_message(
                ...
                reply_markup=get_generic_inline_keyboard()  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
            )
...
async def process_my_profile(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if profile_data:
        ...
        # Редагування існуючого інтерактивного повідомлення з даними профілю
        if interactive_message_id:
            try:
                await check_and_edit_message(
                    ...
                    new_keyboard=get_generic_inline_keyboard(),  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
                    state=state,
                ...
                interactive_message_id = await send_or_update_interactive_message(
                    ...
                    keyboard=get_generic_inline_keyboard(),  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
                    message_id=None,
                    state=state,
                    parse_mode=ParseMode.HTML
                )
        else:
            # Якщо інтерактивне повідомлення не існує, створіть нове
            interactive_message_id = await send_or_update_interactive_message(
                bot=bot,
                chat_id=message.chat.id,
                text=formatted_profile_text,
        ...
        try:
            await bot.send_message(chat_id=message.chat.id, text=error_message, reply_markup=get_generic_inline_keyboard())  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
        except Exception as e:
...
async def cmd_start(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    try:
        interactive_message = await bot.send_message(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_menu()  # Використовуйте ReplyKeyboard замість InlineKeyboard
        )
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text=GENERIC_ERROR_MESSAGE_TEXT,
            reply_markup=get_generic_inline_keyboard()  # Використовуйте відповідну ReplyKeyboard
        )
...
async def handle_intro_next_1(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state != MenuStates.INTRO_PAGE_1.state:
        logger.warning(f"Некоректний стан для 'INTRO_PAGE_1': {current_state}")
    ...
    new_keyboard = get_main_menu()  # Використовуйте відповідну ReplyKeyboard
...
async def handle_intro_next_2(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state != MenuStates.INTRO_PAGE_2.state:
        logger.warning(f"Некоректний стан для 'INTRO_PAGE_2': {current_state}")
        await bot.send_message(message.chat.id, "Некоректна дія для цього стану.", reply_markup=get_generic_inline_keyboard())
        return
...
async def handle_intro_start(message: Message, state: FSMContext, bot: Bot, db: AsyncSession):
    ...
    new_interactive_keyboard = get_generic_inline_keyboard()  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
...
async def handle_change_username(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    try:
        await bot.send_message(chat_id=message.chat.id, text=response_text, reply_markup=get_generic_inline_keyboard())  # Можливо, ця клавіатура тепер буде змінена на відповідну ReplyKeyboard
    except Exception as e:
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        new_main_keyboard = get_hero_class_reply_menu(hero_class)  # Використання ReplyKeyboard
...
async def unknown_command(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state == MenuStates.MAIN_MENU.state:
        ...
        new_main_keyboard = get_hero_class_reply_menu(hero_class)
