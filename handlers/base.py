...
async def handle_main_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.NAVIGATION.value:
        ...
        new_main_text = "🔥 <b>Мета</b> – функція ще в розробці."
        new_main_keyboard = get_navigation_menu()
        new_interactive_text = "Мета"
        new_state = MenuStates.META_MENU
    elif user_choice == MenuButton.M6.value:
        new_main_text = "🏆 <b>M6</b> – функція ще в розробці."
        new_main_keyboard = get_navigation_menu()
        new_interactive_text = "M6"
        new_state = MenuStates.M6_MENU
    elif user_choice == MenuButton.GPT.value:
        new_main_text = "👾 <b>GPT</b> – функція ще в розробці."
        new_main_keyboard = get_navigation_menu()
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        new_main_text = "📊 <b>Порівняння героїв</b> – функція ще в розробці."
        new_main_keyboard = get_heroes_menu()
        ...
        if data == "mls_button":
            ...
            if old_bot_message_id:
                try:
                    ...
                    logger.error(f"Не вдалося видалити повідомлення бота: {e}")
        elif data == "gpt_button":
            await bot.answer_callback_query(callback.id, text=AI_INTRO_TEXT, show_alert=True)
        elif data == "m6_button":
            await bot.answer_callback_query(callback.id, text="🏆 <b>M6</b> – функція ще в розробці.", show_alert=True)
        elif data == "meta_button":
            await bot.answer_callback_query(callback.id, text="🔥 <b>Мета</b> – функція ще в розробці.", show_alert=True)
        else:
