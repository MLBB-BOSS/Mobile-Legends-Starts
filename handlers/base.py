...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    # Відправляємо повідомлення з текстом і звичайною клавіатурою (Повідомлення 1)
...
# Обробник натискання звичайних кнопок
@router.message(F.text)
async def handle_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    # Отримуємо bot_message_id та interactive_message_id з стану
    ...
    # Визначаємо новий текст та клавіатуру для Повідомлення 1 та Повідомлення 2
    new_main_text = ""
    new_main_keyboard = None
    new_interactive_text = ""
    new_interactive_keyboard = None
    new_state = None
    ...
    # Додайте обробку інших станів та кнопок за потребою

    # Редагуємо Повідомлення 1 (бота з клавіатурою)
    try:
    ...
    # Редагуємо Повідомлення 2 (інтерактивне повідомлення)
    try:
        await bot.edit_message_text(
            chat_id=message.chat.id,
...
async def handle_inline_buttons(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if interactive_message_id:
        ...
        # Додайте обробку ваших інлайн-кнопок тут
        if data == "button1":
            # Приклад обробки кнопки
            await bot.answer_callback_query(callback.id, text="Ви натиснули кнопку 1")
        else:
            await bot.answer_callback_query(callback.id, text="Невідома кнопка")
    else:
