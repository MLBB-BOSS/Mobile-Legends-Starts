...
from utils.message_utils import safe_delete_message, check_and_edit_message, handle_error, transition_state
...
from texts import (
    ...
    HEROES_LIST_TEXT  # Додано для відображення списку героїв
)
...
async def handle_main_menu_buttons(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if not bot_message_id or not interactive_message_id:
        ...
        await handle_error(bot, chat_id, MAIN_MENU_ERROR_TEXT, logger)
        await transition_state(state, MenuStates.MAIN_MENU)
        return
    ...
    if user_choice == MenuButton.NAVIGATION.value:
        ...
        new_main_text = M6_INFO_TEXT  # Виправлено на правильний текст
        new_main_keyboard = get_m6_menu()
        new_interactive_text = M6_INFO_TEXT  # Виправлено на правильний текст
        ...
        # Повернення до головного меню (можливо, не потрібне в головному меню)
...
async def handle_navigation_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if not bot_message_id or not interactive_message_id:
        ...
        await handle_error(bot, message.chat.id, MAIN_MENU_ERROR_TEXT, logger)
        await transition_state(state, MenuStates.MAIN_MENU)
        return
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        try:
            ...
            # Встановлюємо стан, щоб дочекатися вводу героїв
...
async def handle_comparison_step_1(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if ',' not in heroes_input:
        ...
        await handle_error(bot, message.chat.id, response_text, logger)
        return
    ...
    # Встановлюємо стан для підтвердження порівняння
    await transition_state(state, MenuStates.COMPARISON_CONFIRM)
...
@router.callback_query(Text(startswith="compare_confirm_"))
async def handle_comparison_confirmation(callback: CallbackQuery, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if data == "compare_confirm_yes":
        ...
        if not hero1_name or not hero2_name:
            ...
            await handle_error(bot, callback.message.chat.id, response_text, logger)
            await transition_state(state, MenuStates.HEROES_MENU)
            await callback.answer()
            return

        # Отримання статистики героїв з бази даних
        # Тут необхідно реалізувати фактичне отримання даних, зараз використовуються фіктивні дані
        hero1_stats = {'kills': 50, 'deaths': 30, 'assists': 100}  # Приклад даних
        hero2_stats = {'kills': 70, 'deaths': 40, 'assists': 120}  # Приклад даних
...
# Обробчик порівняння персонажів (підтвердження)
@router.callback_query(Text(startswith="compare_confirm_"))
async def handle_comparison_confirmation(callback: CallbackQuery, state: FSMContext, db: AsyncSession, bot: Bot):
    data = callback.data
    state_data = await state.get_data()
    ...
    if data == "compare_confirm_yes":
        ...
        # Отримання статистики героїв з бази даних
        # Тут необхідно реалізувати фактичне отримання даних, зараз використовуються фіктивні дані
        # Наприклад, замініть ці дані на реальні запити до вашої бази даних
        hero1_stats = {'kills': 50, 'deaths': 30, 'assists': 100}  # Приклад даних
        hero2_stats = {'kills': 70, 'deaths': 40, 'assists': 120}  # Приклад даних
...
# Обробчик меню "HERO_CLASS_MENU"
@router.message(MenuStates.HERO_CLASS_MENU)
async def handle_hero_class_menu_buttons(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    user_choice = message.text
    logger.info(f"Користувач {message.from_user.id} обрав героя '{user_choice}' у класі героя")
    ...
    if not bot_message_id or not interactive_message_id or not hero_class:
        logger.error("bot_message_id, interactive_message_id або hero_class не знайдено")
        await handle_error(bot, message.chat.id, MAIN_MENU_ERROR_TEXT, logger)
        await transition_state(state, MenuStates.MAIN_MENU)
        return

    # Отримання списку героїв для обраного класу
    try:
        heroes = heroes_by_class(hero_class)  # Припускаємо, що ця функція повертає список героїв
        if not heroes:
            heroes_text = f"❌ У класі **{hero_class}** немає доступних героїв."
        else:
            heroes_text = HEROES_LIST_TEXT.format(hero_class=hero_class, heroes=", ".join(heroes))
    except Exception as e:
        logger.error(f"Помилка при отриманні героїв для класу {hero_class}: {e}")
        heroes_text = GENERIC_ERROR_MESSAGE_TEXT

    # Генерація клавіатури з поверненням
    heroes_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="menu_back")]
    ])

    # Редагування інтерактивного повідомлення з списком героїв
    try:
        await check_and_edit_message(
            bot=bot,
            chat_id=message.chat.id,
            message_id=interactive_message_id,
            new_text=heroes_text,
            new_keyboard=heroes_keyboard,
            state=state,
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
        logger.error(f"Не вдалося редагувати інтерактивне повідомлення з героями: {e}")
        await handle_error(bot, message.chat.id, GENERIC_ERROR_MESSAGE_TEXT, logger)
        return

    # Надсилання нового повідомлення з підтвердженням вибору героя або іншими діями
    try:
        hero_details_text = f"🔍 Детальна інформація про героя **{user_choice}** ще в розробці."
        hero_details_keyboard = get_generic_inline_keyboard()
        hero_details_message = await bot.send_message(
            chat_id=message.chat.id,
            text=hero_details_text,
            reply_markup=hero_details_keyboard,
            parse_mode=ParseMode.MARKDOWN
        )
        new_bot_message_id = hero_details_message.message_id
    except Exception as e:
        logger.error(f"Не вдалося надіслати повідомлення з деталями героя: {e}")
        await handle_error(bot, message.chat.id, GENERIC_ERROR_MESSAGE_TEXT, logger)
        return
    ...
    # Оновлення стану користувача
    await transition_state(state, MenuStates.HERO_CLASS_MENU)
    await state.update_data(bot_message_id=new_bot_message_id)

# Уніфікована функція для обробки меню
# Видалено для уникнення дублювання та конфліктів
...
async def process_my_profile(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if profile_data:
        ...
        await handle_error(bot, message.chat.id, error_message, logger)
        await transition_state(state, MenuStates.MAIN_MENU)

# Обробчик натискання звичайних кнопок у меню Персонажі (клас героїв)
@router.message(MenuStates.HERO_CLASS_MENU)
async def handle_hero_class_menu_buttons(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    user_choice = message.text
    logger.info(f"Користувач {message.from_user.id} обрав героя '{user_choice}' у класі героя")
...
# Обробчик натискання звичайних кнопок у меню Зворотний Зв'язок
...
async def handle_guides_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    elif user_choice == "Розширені Техніки":
        new_main_text = ADVANCED_TECHNIQUES_TEXT
        new_interactive_text = "Розширені техніки"
...
# Обробчик меню "GUIDES Menu" - додаткові дії (якщо потрібно)

# Обробчик меню "HERO_CLASS_MENU" вже доданий вище

# Обробчик для налаштування обробників
