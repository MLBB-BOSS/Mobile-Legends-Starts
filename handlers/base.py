...
async def cmd_guides(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете знайти різні гайди для покращення вашої гри, навчання нових стратегій та вдосконалення навичок.\n\n"
        "Оберіть підрозділ гайдів нижче для подальших дій."
    )
...
async def cmd_counter_picks(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете знайти контр-піки для різних персонажів, щоб підвищити свої шанси на перемогу у матчах.\n\n"
        "Оберіть опцію контр-піків нижче для подальших дій."
    )
...
async def cmd_builds(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете переглядати та створювати білди для різних персонажів, щоб максимально ефективно використовувати їхні навички.\n\n"
        "Оберіть опцію білдів нижче для подальших дій."
    )
...
async def cmd_voting(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете брати участь у різних голосуваннях, ділитися своїми думками та впливати на розвиток спільноти.\n\n"
        "Оберіть опцію голосування нижче для подальших дій."
    )
...
async def cmd_new_guides(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список нових гайдів ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_guides_menu())
...
async def cmd_popular_guides(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список популярних гайдів ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_guides_menu())
...
async def cmd_beginner_guides(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список гайдів для початківців ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_guides_menu())
...
async def cmd_advanced_techniques(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список просунутих технік ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_guides_menu())
...
async def cmd_teamplay_guides(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список гайдів по командній грі ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_guides_menu())
...
async def cmd_counter_search(message: Message, state: FSMContext):
    ...
    description = "⚖️ <b>Пошук Контр-піку:</b>"
    detailed_text = (
        "Введіть ім'я персонажа для пошуку контр-піку. Бот надасть рекомендації для обраного персонажа."
    )
    await send_menu_response(message, description, detailed_text, get_generic_inline_keyboard())
...
async def cmd_counter_list(message: Message, state: FSMContext):
    ...
    description = "⚖️ <b>Список Персонажів:</b>"
    detailed_text = (
        "Список персонажів для контр-піків ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_counter_picks_menu())
...
async def cmd_create_build(message: Message, state: FSMContext):
    ...
    description = "⚜️ <b>Створення Білду:</b>"
    detailed_text = (
        "Функція створення білду ще в розробці. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_builds_menu())
...
async def cmd_my_builds(message: Message, state: FSMContext):
    ...
    description = "⚜️ <b>Мої Білди:</b>"
    detailed_text = (
        "Список ваших білдів ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_builds_menu())
...
async def cmd_popular_builds(message: Message, state: FSMContext):
    ...
    description = "⚜️ <b>Популярні Білди:</b>"
    detailed_text = (
...
async def cmd_current_votes(message: Message, state: FSMContext):
    ...
    description = "📊 <b>Поточні Опитування:</b>"
    detailed_text = (
        "Список поточних опитувань ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_voting_menu())
...
async def cmd_my_votes(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Список ваших голосувань ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_voting_menu())
...
async def cmd_suggest_topic(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Введіть тему для пропозиції. Ваші ідеї допоможуть покращити бот та гру!"
    )
    await send_menu_response(message, description, detailed_text, get_generic_inline_keyboard())
...
async def cmd_statistics(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете переглянути свою загальну статистику, рейтинг та ігрові показники.\n\n"
        "Оберіть підрозділ статистики нижче для подальших дій."
    )
...
async def cmd_achievements(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете переглянути свої досягнення, бейджі та нагороди за успішну гру.\n\n"
        "Оберіть підрозділ досягнень нижче для подальших дій."
    )
...
async def cmd_settings(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете налаштувати різні параметри вашого профілю, такі як мова інтерфейсу, сповіщення та інше.\n\n"
        "Оберіть опцію налаштувань нижче для подальших дій."
    )
...
async def cmd_feedback(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете надсилати відгуки, пропозиції або повідомляти про помилки, які ви знайшли в боті.\n\n"
        "Оберіть опцію зворотного зв'язку нижче для подальших дій."
    )
...
async def cmd_help(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "У цьому розділі ви можете отримати інструкції щодо використання бота, переглянути FAQ або зв'язатися з підтримкою.\n\n"
        "Оберіть опцію допомоги нижче для подальших дій."
    )
...
async def cmd_activity(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Ваші загальні показники активності ще не доступні. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_statistics_menu())
...
async def cmd_game_stats(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Ваша ігрова статистика ще не доступна. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_statistics_menu())
...
async def cmd_badges(message: Message, state: FSMContext):
    ...
    description = "🏆 <b>Мої Бейджі:</b>"
    detailed_text = (
        "Список ваших бейджів ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_achievements_menu())
...
async def cmd_progress(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Ваш прогрес ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_achievements_menu())
...
async def cmd_tournament_stats(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Турнірна статистика ще не доступна. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_achievements_menu())
...
async def cmd_awards(message: Message, state: FSMContext):
    ...
    description = "🏆 <b>Отримані Нагороди:</b>"
    detailed_text = (
        "Список отриманих нагород ще не доступний. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_achievements_menu())
...
async def cmd_language(message: Message, state: FSMContext):
    ...
    description = "⚙️ <b>Мова Інтерфейсу:</b>"
    detailed_text = (
        "Функція зміни мови ще в розробці. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_settings_menu())
...
async def cmd_change_username(message: Message, state: FSMContext):
    ...
    description = "⚙️ <b>Змінити Username:</b>"
    detailed_text = (
...
async def cmd_update_id(message: Message, state: FSMContext):
    ...
    description = "⚙️ <b>Оновити ID Гравця:</b>"
    detailed_text = (
        "Функція оновлення ID ще в розробці. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_settings_menu())
...
async def cmd_notifications(message: Message, state: FSMContext):
    ...
    description = "⚙️ <b>Сповіщення:</b>"
    detailed_text = (
        "Функція налаштування сповіщень ще в розробці. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_settings_menu())
...
async def cmd_send_feedback(message: Message, state: FSMContext):
    ...
    description = "📢 <b>Надіслати Відгук:</b>"
    detailed_text = (
        "Будь ласка, введіть ваш відгук. Ми цінуємо ваші думки та пропозиції!"
    )
    await send_menu_response(message, description, detailed_text, get_generic_inline_keyboard())
...
async def cmd_report_bug(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Будь ласка, опишіть помилку, яку ви знайшли. Ми швидко її виправимо!"
    )
    await send_menu_response(message, description, detailed_text, get_generic_inline_keyboard())
...
async def cmd_instructions(message: Message, state: FSMContext):
    ...
    detailed_text = (
        "Інструкції ще не доступні. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_help_menu())
...
async def cmd_faq(message: Message, state: FSMContext):
    ...
    description = "❓ <b>FAQ:</b>"
    detailed_text = (
        "FAQ ще не доступне. Слідкуйте за оновленнями!"
    )
    await send_menu_response(message, description, detailed_text, get_help_menu())
