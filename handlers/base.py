...
# Відповідність між кнопками та назвами класів героїв
...
async def increment_step(state: FSMContext):
    """
    Лічильник кроків для прикладу або для відстеження, скільки разів
    користувач натискає кнопки. Якщо значення досягає 3, ми знову
    очищуємо state.
    """
...
async def send_or_update_interactive_message(
    ...
    """
    Відправка нового повідомлення з інлайн-клавіатурою або
    оновлення існуючого повідомлення (за message_id).
    """
    if message_id:
        try:
    ...
    # Якщо message_id не задано або редагувати не вийшло — надсилаємо нове
    try:
...
async def safe_delete_message(bot: Bot, chat_id: int, message_id: int):
    """
    Безпечне видалення повідомлення, щоб не викликати помилку,
    якщо воно вже видалено або прострочено.
    """
    if message_id:
        try:
...
async def check_and_edit_message(
    ...
    """
    Редагування повідомлення, тільки якщо змінився текст чи клавіатура.
    Запобігає помилці "Message is not modified", якщо нічого не змінилося.
    """
    state_data = await state.get_data()
    last_text = state_data.get('last_text')
    last_keyboard = state_data.get('last_keyboard')

    # Якщо текст або клавіатура відрізняються від попередніх, намагаємось редагувати
...
async def transition_state(state: FSMContext, new_state: State):
    """
    Очищення попередніх даних та встановлення нового стану.
    """
    await state.clear()
    await state.set_state(new_state)


# --------------------- ДОПОМІЖНА ФУНКЦІЯ: МОЙ ПРОФІЛЬ --------------------- #

async def process_my_profile(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    """
    Винесена в окрему функцію логіка обробки «Мій Профіль».
    1) Отримуємо дані з БД
    2) Видаляємо повідомлення користувача
    3) Якщо профіль є — показуємо, якщо ні — помилка
    """
    user_id = message.from_user.id
    profile_data = await get_user_profile(db, user_id)  # ваша функція з utils.db
    ...
    if profile_data:
        ...
        # Отримуємо з state popередні повідомлення
        data = await state.get_data()
        old_bot_message_id = data.get('bot_message_id')
        interactive_message_id = data.get('interactive_message_id')

        # Оновлюємо або створюємо інтерактивне повідомлення з текстом профілю
        ...
        # Надсилаємо нове звичайне повідомлення «🪪 Мій Профіль»
        try:
...
async def cmd_start(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    """
    Обробляє /start: реєстрація у БД + відправка INTRO_PAGE_1
    """
    user_id = message.from_user.id
    async with db.begin():
...
# --------------------- ОБРОБНИКИ ВСТУПНИХ СТОРІНОК (CALLBACK) --------------------- #

@router.callback_query(F.data == "intro_next_1")
async def handle_intro_next_1(callback: CallbackQuery, state: FSMContext, bot: Bot):
    """
    Перехід з INTRO_PAGE_1 на INTRO_PAGE_2
    """
    ...
    if current_state != MenuStates.INTRO_PAGE_1.state:
        ...
        await bot.answer_callback_query(
            callback.id, text="Некоректна дія для цього стану.", show_alert=True
        )
        return
...
async def handle_intro_next_2(callback: CallbackQuery, state: FSMContext, bot: Bot):
    """
    Перехід з INTRO_PAGE_2 на INTRO_PAGE_3
    """
    current_state = await state.get_state()
...
async def handle_intro_start(callback: CallbackQuery, state: FSMContext, bot: Bot, db: AsyncSession):
    """
    Завершує етап INTRO_PAGE_3 і переходить до MAIN_MENU
    """
    current_state = await state.get_state()
    ...
    # Відправка головного меню (ReplyKeyboard)
    try:
    ...
    # Оновлення/створення інтерактивного повідомлення
    state_data = await state.get_data()
    interactive_message_id = state_data.get('interactive_message_id')
...
# --------------------- ПРИКЛАД ОБРОБНИКА ДЛЯ "МІЙ ПРОФІЛЬ" (КНОПКА) --------------------- #

@router.message(F.text == "🪪 Мій Профіль")
async def handle_my_profile_handler(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    """
    Якщо користувач у будь-якому стані відправив повідомлення з текстом "🪪 Мій Профіль",
    робимо перевірку та обробку профілю.
    """
    await increment_step(state)
    await process_my_profile(message, state, db, bot)


# --------------------- УНІВЕРСАЛЬНА ФУНКЦІЯ ДЛЯ "MAIN_MENU" --------------------- #
...
async def handle_menu(
    ...
    """
    Абстракція для обробки переходів у підменю з MAIN_MENU.
    """
    ...
    if user_choice == MenuButton.NAVIGATION.value:
        ...
        # Викликаємо логіку профілю окремо і припиняємо виконання
...
# --------------------- ОБРОБКА ВВЕДЕННЯ ЗВОРОТНОГО ЗВ'ЯЗКУ --------------------- #
...
async def handle_receive_feedback(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if feedback:
        # Можливе збереження у БД або надсилання адміністратору
...
async def handle_report_bug(message: Message, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    if bug_report:
        # Збереження у БД або надсилання адміністратору
...
# --------------------- ОБРОБНИК НАВІГАЦІЇ (NAVIGATION_MENU) --------------------- #
...
# --------------------- ОБРОБКА МЕНЮ ГЕРОЇ (HEROES_MENU) --------------------- #
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        hero_class = menu_button_to_class.get(user_choice, 'Танк')  # За замовчуванням "Танк"
...
# --------------------- ОБРОБКА УСІХ ІНЛАЙН-КНОПОК --------------------- #

@router.callback_query()
async def handle_inline_buttons(callback: CallbackQuery, state: FSMContext, bot: Bot):
    """
    Ловимо ВСІ callback_query, які не були оброблені вище конкретними фільтрами.
    """
    data = callback.data
    logger.info(f"Користувач {callback.from_user.id} натиснув інлайн-кнопку: {data}")

    # Отримуємо interactive_message_id з state
    state_data = await state.get_data()
    interactive_message_id = state_data.get('interactive_message_id')

    if interactive_message_id:
        if data == "mls_button":
            # Наприклад, кнопка, яка просто показує підказку
            ...
            # Відправляємо головне меню заново
            user_first_name = callback.from_user.first_name or "Користувач"
            main_menu_text_formatted = MAIN_MENU_TEXT.format(user_first_name=user_first_name)
            try:
                main_message = await bot.send_message(
            ...
            # Якщо у нас є інші callback_data або не оброблені
...
async def unknown_command(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state == MenuStates.MAIN_MENU.state:
        ...
        # Якщо ми в процесі введення, нагадуємо про кнопку
        try:
            await bot.send_message(
                chat_id=message.chat.id,
        ...
        # Якщо невідомо де, повертаємо до MAIN_MENU
...
# --------------------- ЗАГАЛЬНА ФУНКЦІЯ ПІДКЛЮЧЕННЯ ЦЬОГО МОДУЛЯ --------------------- #

def setup_handlers(dp: Router):
    dp.include_router(router)
    # Якщо є інші файли-обробники, підключаємо їх тут