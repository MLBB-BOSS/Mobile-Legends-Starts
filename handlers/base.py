...
# Define a parent state mapping
PARENT_STATE_MAP = {
    MenuStates.INTRO_PAGE_1: None,  # No parent
    MenuStates.INTRO_PAGE_2: MenuStates.INTRO_PAGE_1,
    MenuStates.INTRO_PAGE_3: MenuStates.INTRO_PAGE_2,
    MenuStates.MAIN_MENU: None,  # No parent
    MenuStates.NAVIGATION_MENU: MenuStates.MAIN_MENU,
    MenuStates.HEROES_MENU: MenuStates.NAVIGATION_MENU,
    MenuStates.HERO_CLASS_MENU: MenuStates.HEROES_MENU,
    MenuStates.GUIDES_MENU: MenuStates.NAVIGATION_MENU,
    MenuStates.COUNTER_PICKS_MENU: MenuStates.NAVIGATION_MENU,
    MenuStates.BUILDS_MENU: MenuStates.NAVIGATION_MENU,
    MenuStates.VOTING_MENU: MenuStates.NAVIGATION_MENU,
    ...
    MenuStates.SETTINGS_MENU: MenuStates.PROFILE_MENU,
    ...
    MenuStates.SEARCH_TOPIC: MenuStates.VOTING_MENU,
    MenuStates.CHANGE_USERNAME: MenuStates.SETTINGS_MENU,
    MenuStates.RECEIVE_FEEDBACK: MenuStates.FEEDBACK_MENU,
    MenuStates.REPORT_BUG: MenuStates.FEEDBACK_MENU,
    MenuStates.TOURNAMENTS_MENU: MenuStates.MAIN_MENU,
    MenuStates.META_MENU: MenuStates.MAIN_MENU,
    MenuStates.M6_MENU: MenuStates.MAIN_MENU,
}

def get_parent_state(current_state: State):
    return PARENT_STATE_MAP.get(current_state, None)
...
async def handle_main_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.NAVIGATION.value:
        ...
        new_main_text = TOURNAMENTS_MENU_TEXT  # Use predefined text
        new_main_keyboard = get_tournaments_menu()
        new_interactive_text = TOURNAMENTS_INTERACTIVE_TEXT  # Define if necessary
        new_state = MenuStates.TOURNAMENTS_MENU
    elif user_choice == MenuButton.META.value:
        new_main_text = META_MENU_TEXT  # Use predefined text
        new_main_keyboard = get_meta_menu()
        new_interactive_text = META_INTERACTIVE_TEXT  # Define if necessary
        new_state = MenuStates.META_MENU
    elif user_choice == MenuButton.M6.value:
        new_main_text = M6_MENU_TEXT  # Use predefined text
        new_main_keyboard = get_m6_menu()
        new_interactive_text = M6_INTERACTIVE_TEXT  # Define if necessary
...
async def handle_navigation_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.HEROES.value:
        ...
        new_main_keyboard = get_tournaments_menu()
        new_interactive_text = TOURNAMENTS_INTERACTIVE_TEXT
        new_state = MenuStates.TOURNAMENTS_MENU
    elif user_choice == MenuButton.META.value:
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    elif user_choice == "⚖️ Порівняння":  # Adjusted to match MenuButton.COMPARISON.value
    ...
    if interactive_message_id:
        ...
        if data == "mls_button":
            ...
            if not current_state:
                logger.error("Current state not found")
                await bot.answer_callback_query(callback.id, text=GENERIC_ERROR_MESSAGE_TEXT)
                return

            current_state_obj = MenuStates(current_state)

            parent_state = get_parent_state(current_state_obj)
            if parent_state:
                # Set the state to the parent state
                await state.set_state(parent_state)
                new_interactive_text = MAIN_MENU_DESCRIPTION  # Adjust as needed based on parent_state
                ...
                if parent_state == MenuStates.MAIN_MENU:
                ...
                elif parent_state == MenuStates.PROFILE_MENU:
                    main_message = await bot.send_message(
                        chat_id=callback.message.chat.id,
                        text=PROFILE_MENU_TEXT,
                        reply_markup=get_profile_menu()
                    )
                elif parent_state == MenuStates.NAVIGATION_MENU:
                    main_message = await bot.send_message(
                        chat_id=callback.message.chat.id,
                    ...
                    # Handle other parent states as needed
                    main_message = await bot.send_message(
                        chat_id=callback.message.chat.id,
                        text=MAIN_MENU_TEXT.format(user_first_name=callback.from_user.first_name),
                        reply_markup=get_main_menu()
                    )
                ...
                # No parent state defined
                logger.error(f"No parent state defined for {current_state_obj}")
                await bot.answer_callback_query(callback.id, text=GENERIC_ERROR_MESSAGE_TEXT)
        else:
...
async def handle_search_hero(message: Message, state: FSMContext, bot: Bot):
    ...
    # Return the user to the previous menu based on parent state
    current_state = await state.get_state()
    parent_state = get_parent_state(MenuStates.SEARCH_HERO)
