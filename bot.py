...
from data.characters_descriptions import CHARACTER_DESCRIPTIONS
...
async def handle_feedback_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.SEND_FEEDBACK.value:
        new_main_text = "Будь ласка, залиште свій відгук нижче."
...
async def handle_navigation_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.HEROES.value:
        new_main_text = "Оберіть персонажа з наступного списку:"
        new_main_keyboard = get_heroes_menu()
        new_interactive_text = "Персонажі"
        ...
        new_main_text = "Оберіть опцію Контр-піків:"
        new_main_keyboard = get_counter_picks_menu()
        new_interactive_text = "Контр-піки"
        new_state = MenuStates.COUNTER_PICKS_MENU
    elif user_choice == MenuButton.BUILDS.value:
        new_main_text = "Оберіть опцію Білдів:"
        new_main_keyboard = get_builds_menu()
        new_interactive_text = "Білди"
        new_state = MenuStates.BUILDS_MENU
    elif user_choice == MenuButton.VOTING.value:
        new_main_text = "Оберіть опцію Голосування:"
        new_main_keyboard = get_voting_menu()
        new_interactive_text = "Голосування"
...
async def handle_heroes_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice in hero_classes:
        ...
        new_main_text = f"Оберіть персонажа класу {hero_class}:"
        new_main_keyboard = get_hero_class_menu(hero_class)
        new_interactive_text = f"Клас: {hero_class}"
...
# Обробник натискання звичайних кнопок у меню Класу Персонажа
...
async def handle_hero_class_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    data = await state.get_data()
    hero_class = data.get('hero_class', 'Невідомий')
    ...
    if user_choice == Characters.BACK.value:
        # Повернення до меню вибору класу персонажа
        new_main_text = HEROES_MENU_TEXT
        new_main_keyboard = get_heroes_menu()
        new_interactive_text = HEROES_INTERACTIVE_TEXT
        new_state = MenuStates.HEROES_MENU
    elif user_choice in Characters.__members__.values():
        # Якщо користувач обрав персонажа
        character = user_choice
        description = CHARACTER_DESCRIPTIONS.get(character, "Опис персонажа відсутній.")
        new_main_text = f"Опис персонажа {character}:\n\n{description}"
        new_main_keyboard = get_hero_class_menu(hero_class)  # Залишаємо меню класу
        new_interactive_text = f"Інформація про {character}"
        new_state = MenuStates.HERO_CLASS_MENU
    else:
        # Невідома команда
        new_main_text = UNKNOWN_COMMAND_TEXT
...
async def handle_counter_picks_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.COUNTER_SEARCH.value:
        new_main_text = "Введіть ім'я героя для пошуку контр-піку:"
        new_main_keyboard = ReplyKeyboardRemove()
        new_interactive_text = "Пошук контр-піку"
        new_state = MenuStates.SEARCH_HERO
    elif user_choice == MenuButton.COUNTER_LIST.value:
        new_main_text = "Список контр-піків: [Тут буде список]"
...
async def handle_builds_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.CREATE_BUILD.value:
        new_main_text = "Створення білду: [Тут буде форма або інструкція]"
        new_interactive_text = "Створення білду"
    elif user_choice == MenuButton.MY_BUILDS.value:
        new_main_text = "Мої білди: [Тут буде список ваших білдів]"
        new_interactive_text = "Мої білди"
    elif user_choice == MenuButton.POPULAR_BUILDS.value:
        new_main_text = "Популярні білди: [Тут буде список популярних білдів]"
...
async def handle_voting_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.CURRENT_VOTES.value:
        new_main_text = "Поточні опитування: [Тут буде список поточних опитувань]"
        new_interactive_text = "Поточні опитування"
    elif user_choice == MenuButton.MY_VOTES.value:
        new_main_text = "Мої голосування: [Тут буде список ваших голосувань]"
...
async def handle_statistics_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.ACTIVITY.value:
        new_main_text = "Загальна активність: [Тут буде статистика активності]"
        new_interactive_text = "Загальна активність"
    elif user_choice == MenuButton.RANKING.value:
        new_main_text = "Рейтинг: [Тут буде ваш рейтинг]"
        new_interactive_text = "Рейтинг"
    elif user_choice == MenuButton.GAME_STATS.value:
        new_main_text = "Ігрова статистика: [Тут буде ваша ігрова статистика]"
...
async def handle_settings_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.LANGUAGE.value:
        new_main_text = "Виберіть мову інтерфейсу:"
        # Тут можна додати меню вибору мови
        ...
        new_main_text = "Ваш ID оновлено: [Тут буде ваш ID]"
        new_interactive_text = "Оновити ID гравця"
    elif user_choice == MenuButton.NOTIFICATIONS.value:
        new_main_text = "Сповіщення: [Тут буде налаштування сповіщень]"
...
async def handle_hero_class_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == Characters.BACK.value:
        ...
        new_interactive_text = NAVIGATION_INTERACTIVE_TEXT
        new_state = MenuStates.NAVIGATION_MENU
    elif user_choice in [character.value for character in Characters if character != Characters.BACK]:
        # Якщо користувач обрав персонажа
        character = user_choice
        description = CHARACTER_DESCRIPTIONS.get(character, "Опис персонажа відсутній.")
        new_main_text = f"Опис персонажа {character}:\n\n{description}"
...
async def handle_help_menu_buttons(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_choice == MenuButton.INSTRUCTIONS.value:
        new_main_text = "Інструкції: [Тут буде текст з інструкціями]"
        new_interactive_text = "Інструкції"
    elif user_choice == MenuButton.FAQ.value:
        new_main_text = "FAQ: [Тут буде список найпоширеніших питань]"
        new_interactive_text = "FAQ"
    elif user_choice == MenuButton.HELP_SUPPORT.value:
        new_main_text = "Підтримка: Зв'яжіться з нами за адресою support@example.com"
...
async def handle_search_hero(message: Message, state: FSMContext, bot: Bot):
    ...
    if hero_name:
        description = CHARACTER_DESCRIPTIONS.get(hero_name, "Персонаж не знайдено або опис відсутній.")
        response_text = f"Опис героя {hero_name}:\n\n{description}"
    else:
...
async def handle_search_topic(message: Message, state: FSMContext, bot: Bot):
    ...
    if topic:
        response_text = f"Дякуємо за пропозицію теми: {topic}"
    else:
...
async def handle_change_username(message: Message, state: FSMContext, bot: Bot):
    ...
    if new_username:
        response_text = f"Ваш новий Username: {new_username}"
    else:
...
async def handle_receive_feedback(message: Message, state: FSMContext, bot: Bot):
    ...
    if feedback:
        response_text = "Дякуємо за ваш відгук!"
    else:
...
async def handle_report_bug(message: Message, state: FSMContext, bot: Bot):
    ...
    if bug_report:
        response_text = "Дякуємо за повідомлення про помилку! Ми розглянемо ваш звіт."
    else:
